...
Try {
	...
	[string]$appVendor = 'Inspiration Software'
	[string]$appName = 'Inspiration and Kidspiration'
	[string]$appVersion = '9.2 and 3.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '3.7.0.1'
	[string]$appScriptDate = '07/22/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'kids3,insp9' -CheckDiskSpace -PersistPrompt
        ...
        # To generate, unpackage zip, run 'setup.exe -r: settings.ini' to generate ini.
        # Then run the app itself, insert licensing info, and copy out the NOS file in install directory

	    Copy-File -Path "$dirFiles\Inspiration\Inspiration NOS" -Destination "C:\Program Files (x86)\Inspiration 9\"
			Execute-Process -Path "$dirFiles\Inspiration\SETUP.EXE" -Parameters '-p settings.ini' -WindowStyle 'Hidden'
			Start-Sleep -seconds 5
			Stop-Process -Name "Insp9" -Force

	    Copy-File -Path "$dirFiles\Kidspiration\Kidspiration NOS" -Destination "C:\ProgramData\Kidspiration 3\"
			Execute-Process -Path "$dirFiles\Kidspiration\Kidspiration 3 Installer.exe" -Parameters '-p settings.ini' -WindowStyle 'Hidden'
			Start-Sleep -seconds 5
			Stop-Process -Name  "Kids3" -Force
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'kids3,insp9' -CloseAppsCountdown 60
		...
		#Uninstall Inspiration - Make sure NOT to quote around the path. I know. It's dumb.
		Execute-Process -Path 'C:\WINDOWS\unvise32.exe' -Parameters '/s C:\Program Files (x86)\Inspiration 9\uninstal.log'

		#Uninstall NovaPDF (bundled with Inspiration)
		Execute-Process -Path 'C:\Program Files\Softland\novaPDF 7\unins000.exe' -Parameters '/NORESTART /VERYSILENT'

		#Uninstall Kidspiration
		Execute-Process -Path 'C:\WINDOWS\unvise32.exe' -Parameters '/s C:\ProgramData\Kidspiration 3\uninstal.log'
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBpeg1CI23Dq+Xm
# D+X+gQ3+sXtHTCiN+5koajlMiJzTLKCCFFwwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgrNicT4ob
# R/xkYrpGXzA781Bb5hgtOZEathVYI5haxG0wDQYJKoZIhvcNAQEBBQAEggEAgVVl
# SeEtP8YDKgSBdY1w/oBydA5FDvepl3iCZV6hO4AcDYP82pp/XlQP3QkyOBheCfVW
# 5n3nhx1zg2E+aOcdWOM2d1YfR2kEvzOs5wBDVMPNaRYv11Lwqle5hgNQt5tAU/Po
# mlHqMgBD2lnHnp4Vt0DgrBw3uLAdFtB8uLiNrJUDxXsU1puoKkpjSOIEE0PJPJ3l
# oAitqRgOM/yIG5sBLURsUSDxxn2hwUhGobH8sYDNHZsDHASZc67VZ9hnZrpIynGC
# J3F0sg5f/w9A9mTKgw/fnTQaT9lUAB66V+I7DJwxP2v7JZoULLPgDQHuLCWsHkSf
# rxUadti8u1nzytYJTKGCAqIwggKeBgkqhkiG9w0BCQYxggKPMIICiwIBATBoMFIx
...
# CSqGSIb3DQEJBTEPFw0xOTA3MjMxNDMyMDBaMCMGCSqGSIb3DQEJBDEWBBTaxoNz
# lf5ChAWW6HpkOKaBeQ9v/zCBnQYLKoZIhvcNAQkQAgwxgY0wgYowgYcwgYQEFGO4
...
# ggEAPeW0sYPTf0DCQPg/Js4NARKjSd/GXYcyY7qP1i0pFVdJaFh5Ba6XtCx9APYU
# 6mGsu8sD3thdg37BBWU36wYR0QvBNWI8vwrUDgV/32Vk8AjuKb3BlZ9cesBxflLU
# A2L8+3jSkjE3pgZDGnvAKTR+bDfbSR+PzHut0hyhdeoiy2tDZFETqyv1I9NmnBOn
# gxisUfy6ANUe41t5sLZ9x0wqtl0RLuSHOyHNgD0oKcfFKYdVtDWVGRGZPPTD7KwA
# TbqSme+36ST36KZgVHTo2rLD6AAmKSMENDMrQWtCQGQiPHRRZgwUW+N5abwmmJsL
# f0uhdkokd7lbnfQr276fQOSPVA==
